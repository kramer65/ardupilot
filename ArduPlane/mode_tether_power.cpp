...
bool ModeTetherPower::_enter()
{
    ...
    if (hal.util->was_watchdog_armed()) {  // If watchdog was not armed it suggests that the autopilot may have experienced a watchdog reset and may be in an unexpected state. Therefore, this check is a safety feature to ensure that the autopilot is in a known state before continuing with the mission.
...
// bool ModeTetherPower::does_auto_navigation() const
// {
...
// bool ModeTetherPower::does_auto_throttle() const
// {
// // #if AP_SCRIPTING_ENABLED
// //    return (!plane.nav_scripting_active());
// // #endif
...
// bool ModeTetherPower::_pre_arm_checks(size_t buflen, char *buffer) const
// {
